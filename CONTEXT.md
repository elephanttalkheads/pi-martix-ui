...
**培育仓盘位**：
Power Plant 环带上的物理容量单位；它可以为空，不等同于会话，也不随会话数量虚构。
_Avoid_: 休眠会话、伪会话

**会话培育仓**：
由一个真实会话占用的培育仓盘位，是该会话在侧栏中的视觉实体；空盘位不得称为会话培育仓。
_Avoid_: 会话卡、session pod、空仓

**项目转环**：
Power Plant 中与一个工作区对应、可绕建筑内部竖直主轴独立旋转的完整环层；纵向承重主轴保持静止，切换该工作区会话时旋转项目转环，而不是让单个培育仓自行转动。
_Avoid_: 旋转墙、整栋旋转、仓体转盘

**盘位总成**：
项目转环上的刚性重复单元，由旋转墙段、机械接口或管线以及一个培育仓盘位共同组成；三者保持相同的安装角和运动关系。所有盘位总成共享项目转环的旋转偏移量，空盘位也保留其墙段与接口结构。
_Avoid_: 单独培育仓、连接线、墙面装饰

**轴承剖面**：
项目转环的默认观察形态；通过减少固定外壳遮挡，显露局部轴承、旋转墙段、机械接口和培育仓的刚性关系，但不展示完整圆盘或完整圆环。
_Avoid_: 诊断视图、透明模式、完整转盘
...
**工具链块（trace）**：
feed 中表示一次工具调用生命周期（开始→结束）的机械继电器单元：DIN 导轨上一枚继电器——触点 LED 三态（执行中琥珀线圈呼吸 / 完成绿 + clack 冲击波 / 失败红）+ `[tag] 描述` + 数码管耗时读数，点击展开参数抽屉（参数全文）。
_Avoid_: 工具卡、命令卡片

**机械继电器**：
工具链块的视觉隐喻（3.3C）：工具调用是「能量被导入一个真实模块」——导轨即载体，触点 LED 即状态机，收尾 clack 冲击波即合闸。
_Avoid_: 液态玻璃卡、涟漪（已退役的旧视觉）

**脑波褶**：
思考块的视觉隐喻（3.2A）：思维链是「正在发生的认知过程」——summary 旁活体脑波折线（流式思考中流动 + 振幅呼吸），思考体越新越亮（末几行沉降梯度，旧念头自然变暗）。
_Avoid_: 加载动画、spinner

**烧录显影**：
diff 卡的入场仪式（3.4A）：一次物理写入在文件上留下的痕迹——新增行逐行白热闪光冷却成绿、删除行焦化落余烬，全部落定后边框自绘一周（校验环闭合）。
_Avoid_: glitch 扫入（已退役的旧入场）

**封存带**：
结算行的视觉形态（3.5C）：回合闭环是「给这次执行盖上封印」——◆ 锚点后封存带向右展开，正常收尾带尾 EOL 方块闪两下；中断/错误版带尾撕裂锯齿、无 EOL——「被封存」与「被撕断」形态一眼可辨。
_Avoid_: 火漆印、沉降件

**磁带纹**：
凝结雨轨曾采用的纹理变体（3.6C：短划纹持续上行，闭环定格压 ◆）——落地后已退回迷你数字雨形态，词条保留作决策记录。
_Avoid_: 迷你数字雨（现行形态）

**字形蛾**：
流式光标的视觉形态（3.7B）：光标是「写入头」——一只高速翻滚的 Matrix 字形蛾，它活着文本就在生长；回合闭环或中断即熄灭。中断标记（[已被操作员中断]）以乱码逐位锁定入场，与注入解码同语言。
_Avoid_: 方块光标、闪烁竖线
...
**diff 卡**：
feed 中展示一次真实文件修改的卡片：行号列 + 红删/绿增符号列 + 代码列，蠕虫命中后以烧录显影入场。增删是符号+颜色双编码。
_Avoid_: 变更记录、补丁
...
**凝结雨轨**：
agent 回合左侧的活动指示轨：回合流式期间一条迷你数字雨下落，回合闭环时雨消散、凝结为 ◆。是回合活动状态的可视化，不承载额外业务信息；同一时刻只有活动回合持有雨轨 canvas。（磁带纹变体曾短暂落地，已退回本形态。）
_Avoid_: 进度条、loading 条

**结算行**：
回合闭环时的统计尾行：`◆ + 封存带（已结算/已中断/错误 · N tools · Σtokens · 耗时）`（tokens 为回合内各 LLM turn 的 usage 求和，耗时为 agent_start→agent_end 实测）。中断回合与错误回合照常结算，带尾撕裂锯齿、标「已中断」/「错误」。
_Avoid_: 汇总、footer
