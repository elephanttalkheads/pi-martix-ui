...
**神经签名**：
会话脑机链路的六种稳定视觉身份之一；同一会话保持相同的字符节奏与节点特征，但不占有固定屏幕槽位，也不限制会话总数。
_Avoid_: 第几条线、固定线路、物理插槽
...
**脉冲包**：
会话脑机链路双向握手的出站半程——从 Neo 发往会话培育仓的短促信号；出站期间链路上的静态字符流让位。
_Avoid_: 信号包、数据包

**回传包**：
会话脑机链路双向握手的返程半程——培育仓收讫脉冲包后向 Neo 回送的应答，生长（尾锚定仓体、头伸向 Neo）→ 维持（两端锚定、内容流向 Neo）→ 收缩（头锚定 Neo、长度归零）三段；与脉冲包永不同屏。
_Avoid_: 响应包、ACK
...
- 会话脑机链路不是纯氛围资产：它可以冗余呼应当前会话和指向状态，但正式状态仍须由名称牌、文字与可访问性语义表达。
