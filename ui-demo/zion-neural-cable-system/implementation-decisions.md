...
- Only the active session runs a five-phase handshake: a short 4-glyph pulse travels from Neo to the pod at approximately `320 px/s` while the static glyph stream yields; once the pulse tail reaches the pod, the return stream grows — tail anchored at the pod, head stretching toward Neo at approximately `140 px/s` until it spans the cable; it then holds for `1 s` with both ends anchored while glyph content scrolls toward Neo at approximately `90 px/s`; then it shrinks — head anchored at Neo, tail chasing Neo at approximately `240 px/s` until its length reaches zero; finally the cable rests for `0.6 s`. The two directions never share the cable at the same time. The return glyph pool is sized dynamically to `ceil(pathLength / 8) + 2`. The SVG path and dormant glyph layout remain defined from Neo to pod.
