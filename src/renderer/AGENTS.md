...
React 18 + TypeScript(strict) 渲染层：v4 四区 UI（标题栏 / 侧栏 / 对话区 / 日志抽屉+状态栏）、agent 事件流接线、扩展 UI 弹层（AskDialog + toast）、氛围动画（数字雨 / 蠕虫入侵 / 神经核心）、WebAudio 音效。不做：IPC 通道与凭据（主进程 `main.mjs`）、preload 桥（`preload.cjs`）、协议类型定义（`src/shared/protocol.ts` 是 type-only 依赖）、扩展对话框的 Promise 表/超时兜底（主进程 `src/main/uibridge.mjs`，本模块只经 `window.zion` 消费）。
...
- `src/renderer/src/components/` — `RainCanvas` / `SignalCanvas`(releaseWorm) / `NeuralCore`(CORE) / `Sidebar`(会话堆叠卡+悬停操作：✎ 重命名 / ✕ 两段删除确认+文件树) / `LogDrawer` / `Feed`(工具链块 step 行可展开) / `DiffCard` / `AskDialog`(扩展对话框三形态 confirm/input/select + `ToastHost` toast 队列) / `InputBar`(快捷指令+命令面板：`/` 弹出 skills/命令 listbox，面板 state 全在组件本地) / `SoundFx`(SND + useSoundFx)
