...
4. **动画/音效数值照规格原样提取**（FS=18、拖尾 0.035、`90/fx.speed` 节流、12% 亮头、L 路径 8px 采样、TAIL=18、扰码 620ms、闪烁 900ms、SND 7 音参数、雨轨 11px 双列）；agent 回复重构数值照 `ui-demo/agent-reply-combo-proto.html`（显影 0.85s、脑波 0.8s/1.6s、烧录 90ms 阶梯封顶 30 行 + 校验环 1.2s、封存带 0.65s + EOL 0.9s×2、字形蛾 120ms 换字形 + 1.1s 呼吸、中断乱码锁定 450ms、注入解码 `min(700, 240+len*6)`ms；3.6C 磁带纹已退回迷你数字雨轨，不引用其数值）：禁止"优化"数值（ADR 0002）。
