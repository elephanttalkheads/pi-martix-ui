...
本模块只承载渲染进程 ↔ 主进程 IPC 的 TypeScript 类型契约（`ZionAPI` 桥面、`AgentSessionEvent` 事件、`FileNode`/`SessionInfoLike`/`SessionHistoryItem`/`ProjectInfo`/`SwitchProjectResult`/`UiAsk`/`UiNotify` 数据形状），运行时零产物：所有消费方只做类型引用。
