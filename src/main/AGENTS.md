...
- **命令清单与执行器两处同步**：面板数据源（`skillscan.mjs` 的 `BUILTIN_COMMANDS` 14 个 + `EXTENSION_COMMANDS` /goal）与执行器（`main.mjs` 的 `commandHandlers`）的命令名必须一一对应——清单只管展示、dispatch 才执行；新增/改名命令两处同改（运行时注册的命令无法静态枚举，漏加则面板不显示或执行报「未知命令」）。带参数命令在清单补 `argumentHint`；未实现命令（trust/hotkeys/model/settings）以占位错误兜底，不得静默假执行。升级 pi SDK 后核对 `BUILTIN_COMMANDS` 是否漂移（裁剪清单见 DESIGN.md 命令聚合节）
