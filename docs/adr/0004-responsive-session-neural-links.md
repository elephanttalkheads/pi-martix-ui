...
5. **动画有界**：状态优先级为 `active > hover/focus > dormant > hidden`。仅可见 active 链路运行双向握手：18 字符脉冲包以约 180px/s 从 Neo 顺向发往培育仓，静态字符流全程让位；脉冲尾端到达仓体后同一帧触发回传包，以约 120px/s 反向返回 Neo；回传走完休止 1.2s，休止期链路只剩线床/神经线/环。两个方向的包永不同屏；SVG path 按 Neo → 仓体定义，dormant 静态线路不反转。hover/focus 只增亮静态线，dormant 完全静止，reduced-motion 关闭脉冲/回传和淡入淡出。
