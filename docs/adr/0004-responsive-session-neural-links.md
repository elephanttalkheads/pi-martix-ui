...
5. **动画有界**：状态优先级为 `active > hover/focus > dormant > hidden`。仅可见 active 链路运行五段握手：4 字符短脉冲以约 320px/s 从 Neo 发往培育仓（静态字符流全程让位）；脉冲尾端到达仓体后回传流进入生长相——尾部锚定仓体、头部以约 140px/s 伸向 Neo 直至铺满全缆；随后两端锚定维持 1s 传输（字符位置固定、内容异相突变，一道连续亮度波从仓体扫向 Neo 恰好一次）；再进入收缩相——头部锚定 Neo、尾部以约 240px/s 脱离仓体追向 Neo 直至长度归零；最后休止 0.6s。两个方向的信号永不同屏；回传字符池按 `ceil(路径长/8)+2` 动态分配；SVG path 按 Neo → 仓体定义，dormant 静态线路不反转。hover/focus 只增亮静态线，dormant 完全静止，reduced-motion 关闭脉冲/回传和淡入淡出。
