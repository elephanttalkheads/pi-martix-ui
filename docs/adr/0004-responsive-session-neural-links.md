...
5. **动画有界**：状态优先级为 `active > hover/focus > dormant > hidden`。仅可见 active 链路从培育仓端点向 Neo 回传约 180px/s 的 18 字符信号包，尾部完全离开 Neo 后休止 1.2s；SVG path 仍按 Neo → 仓体定义，静态线路不反转。hover/focus 只增亮静态线，dormant 完全静止，reduced-motion 关闭脉冲和淡入淡出。
